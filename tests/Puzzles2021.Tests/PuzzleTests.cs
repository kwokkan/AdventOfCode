using AdventOfCode.PuzzleTestCore;

namespace AdventOfCode.Puzzles2021.Tests
{
    public class Puzzle1Tests : TestBase<Puzzle1>
    {
        public Puzzle1Tests()
            : base(7, 5, 1681, 1704)
        {
        }
    }

    public class Puzzle2Tests : TestBase<Puzzle2>
    {
        public Puzzle2Tests()
            : base(150, 900, 1383564, 1488311643)
        {
        }
    }

    public class Puzzle3Tests : TestBase<Puzzle3>
    {
        public Puzzle3Tests()
            : base(198, 230, 1092896, 4672151)
        {
        }
    }

    public class Puzzle4Tests : TestBase<Puzzle4>
    {
        public Puzzle4Tests()
            : base(4512, 1924, 5685, 21070)
        {
        }
    }

    public class Puzzle5Tests : TestBase<Puzzle5>
    {
        public Puzzle5Tests()
            : base(5, 12, 5835, 17013)
        {
        }
    }

    public class Puzzle6Tests : TestBase<Puzzle6>
    {
        public Puzzle6Tests()
            : base(5934, 26984457539, 385391, 1728611055389)
        {
        }
    }

    public class Puzzle7Tests : TestBase<Puzzle7>
    {
        public Puzzle7Tests()
            : base(37, 168, 347011, 98363777)
        {
        }
    }

    public class Puzzle8Tests : TestBase<Puzzle8>
    {
        public Puzzle8Tests()
            : base(26, 61229, 301, 908067)
        {
        }
    }

    public class Puzzle9Tests : TestBase<Puzzle9>
    {
        public Puzzle9Tests()
            : base(15, null, 558, 0)
        {
        }
    }

    public class Puzzle10Tests : TestBase<Puzzle10>
    {
        public Puzzle10Tests()
            : base(26397, 288957, 311895, 2904180541)
        {
        }
    }

    public class Puzzle13Tests : TestBase<Puzzle13>
    {
        public Puzzle13Tests()
            : base(17, 16, 743, 94)
        {
        }
    }

    public class Puzzle14Tests : TestBase<Puzzle14>
    {
        public Puzzle14Tests()
            : base(1588, 2188189693529, 2975, 0)
        {
        }
    }

    public class Puzzle16Tests : TestBase<Puzzle16>
    {
        public Puzzle16Tests()
            : base(31, 54, 913, 1510977819698)
        {
        }
    }

    public class Puzzle17Tests : TestBase<Puzzle17>
    {
        public Puzzle17Tests()
            : base(45, 112, 8256, 2326)
        {
        }
    }

    public class Puzzle21Tests : TestBase<Puzzle21>
    {
        public Puzzle21Tests()
            : base(739785, null, 513936, 0)
        {
        }
    }
}
